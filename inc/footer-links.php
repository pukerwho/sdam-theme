...
function get_term_keywords() {
  ...
  if (get_locale() === 'uk') {
    $anchors = ["Аренда квартири в місті ", "Зняти квартиру в м.", "Довгострогова аренда квартири у місті ", "Скількі коштує зняти квартиру в м."];
  }
  else {
    $anchors = ["Аренда квартиры в городе ", "Снять квартиру в г.", "Долгосрочная аренда квартиры в городе ", "Сколько стоит снять квартиру в г."];
  }
  
  $cities = get_terms(  array('taxonomy' => 'city', "parent" => 0) );
  shuffle( $cities );
  
  foreach ($cities as $city) {
    if (count($terms_links_array) < $count_term_in_array) {
      $city_id = $city->term_id;
      $city_url = get_term_link($city->term_id, 'city');
      $city_name = $city->name;
      $anchor = $anchors[array_rand($anchors, 1)] . $city_name;
      $ahref = '<a href="'.$city_url.'">'.$anchor.'</a>';
      if (!in_array($city_url, $terms_url_array)) {
        array_push($terms_links_array, $ahref); 
        array_push($terms_url_array, $city_url); 
      }
    } else {
